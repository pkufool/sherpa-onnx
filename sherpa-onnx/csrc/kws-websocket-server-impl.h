// sherpa-onnx/csrc/kws-websocket-server-impl.h
//
// Copyright (c)  2022-2023  Xiaomi Corporation

#ifndef SHERPA_ONNX_CSRC_KWS_WEBSOCKET_SERVER_IMPL_H_
#define SHERPA_ONNX_CSRC_KWS_WEBSOCKET_SERVER_IMPL_H_
...
struct KwsWebsocketDecoderConfig {
  KeywordSpotterConfig kws_config;
...
class KwsWebsocketDecoder {
 ...
 private:
  KwsWebsocketServer *server_;  // not owned
  std::unique_ptr<KeywordSpotter> keyword_spotter_;
  KwsWebsocketDecoderConfig config_;
...
struct KwsWebsocketServerConfig {
  KwsWebsocketDecoderConfig decoder_config;
...
class KwsWebsocketServer {
 public:
  explicit KwsWebsocketServer(asio::io_context &io_conn,  // NOLINT
                              asio::io_context &io_work,  // NOLINT
                              const KwsWebsocketServerConfig &config);

  void Run(uint16_t port);

  const KwsWebsocketServerConfig &GetConfig() const { return config_; }
 ...
 private:
  KwsWebsocketServerConfig config_;
...
#endif  // SHERPA_ONNX_CSRC_KWS_WEBSOCKET_SERVER_IMPL_H_
