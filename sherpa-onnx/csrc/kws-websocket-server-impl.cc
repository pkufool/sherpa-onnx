// sherpa-onnx/csrc/kws-websocket-server-impl.cc
//
// Copyright (c)  2022-2023  Xiaomi Corporation

#include "sherpa-onnx/csrc/kws-websocket-server-impl.h"
...
void KwsWebsocketDecoderConfig::Register(ParseOptions *po) {
  kws_config.Register(po);
...
void KwsWebsocketDecoderConfig::Validate() const {
  kws_config.Validate();
...
void KwsWebsocketServerConfig::Register(sherpa_onnx::ParseOptions *po) {
...
void KwsWebsocketServerConfig::Validate() const { decoder_config.Validate(); }

KwsWebsocketDecoder::KwsWebsocketDecoder(KwsWebsocketServer *server)
    : server_(server),
      config_(server->GetConfig().decoder_config),
      timer_(server->GetWorkContext()) {
  keyword_spotter_ = std::make_unique<KeywordSpotter>(config_.kws_config);
}
...
void KwsWebsocketDecoder::AcceptWaveform(std::shared_ptr<Connection> c) {
  std::lock_guard<std::mutex> lock(c->mutex);
  float sample_rate = config_.kws_config.feat_config.sampling_rate;
...
void KwsWebsocketDecoder::InputFinished(std::shared_ptr<Connection> c) {
  std::lock_guard<std::mutex> lock(c->mutex);
...
void KwsWebsocketDecoder::ProcessConnections(const asio::error_code &ec) {
  ...
  for (auto &p : connections_) {
    ...
    if (!keyword_spotter_->IsReady(c->s.get()) && !c->eof) {
...
void KwsWebsocketDecoder::Decode() {
  ...
  lock.unlock();
  keyword_spotter_->DecodeStreams(s_vec.data(), s_vec.size());
  lock.lock();

  for (auto c : c_vec) {
    auto result = keyword_spotter_->GetResult(c->s.get());
...
KwsWebsocketServer::KwsWebsocketServer(asio::io_context &io_conn,
                                       asio::io_context &io_work,
                                       const KwsWebsocketServerConfig &config)
    : config_(config),
      io_conn_(io_conn),
      io_work_(io_work),
...
void KwsWebsocketServer::Run(uint16_t port) {
  server_.set_reuse_addr(true);
  server_.listen(asio::ip::tcp::v4(), port);
  server_.start_accept();
  auto kws_config = config_.decoder_config.kws_config;
  decoder_.Run();
}

void KwsWebsocketServer::SetupLog() {
...
void KwsWebsocketServer::Send(connection_hdl hdl, const std::string &text) {
...
void KwsWebsocketServer::OnOpen(connection_hdl hdl) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void KwsWebsocketServer::OnClose(connection_hdl hdl) {
  std::lock_guard<std::mutex> lock(mutex_);
  connections_.erase(hdl);
...
bool KwsWebsocketServer::Contains(connection_hdl hdl) const {
  std::lock_guard<std::mutex> lock(mutex_);
...
void KwsWebsocketServer::Close(connection_hdl hdl,
